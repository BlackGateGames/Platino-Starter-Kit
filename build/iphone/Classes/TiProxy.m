...
//Rare exceptions to indicate a bug in the _platinostarterkit code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_platinostarterkit = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_platinostarterkit]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.platinostarterkit.module"
