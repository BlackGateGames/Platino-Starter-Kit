...
NSString * PlatinoStarterKit$ModuleRequireFormat = @"(function(exports){"
...
@implementation PlatinoStarterKitObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_platinostarterkit);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create PlatinoStarterKit global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "PlatinoStarterKit" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_platinostarterkit = [[PlatinoStarterKitObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_platinostarterkit];
	
	NSString *_platinostarterkitNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _platinostarterkitNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_platinostarterkit valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:PlatinoStarterKit$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.platinostarterkit.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.platinostarterkit.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
